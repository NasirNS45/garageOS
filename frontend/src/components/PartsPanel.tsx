...
export default function PartsPanel({ card, isEditable }: Props) {
  ...
  const handleAdd = () => {
    ...
    if (!name.trim()) { setAddError("Part name is required"); return; }
    if (!price || isNaN(unit_price) || unit_price < 0) { setAddError("Enter a valid price"); return; }
    if (isNaN(quantity) || quantity <= 0) { setAddError("Enter a valid quantity"); return; }
    setAddError("");
  ...
  const saveEdit = () => {
    ...
    updatePart.mutate(
      ...
      {
        onSuccess: () => { setEditingId(null); toast("Part updated", "success"); },
        onError: () => toast("Failed to update part", "error"),
      }
    );
  };
      ...
      {card.parts.length > 0 && (
        <div className="space-y-1.5 mb-2">
          ...
          <div className="flex justify-end text-xs font-semibold text-slate-600 dark:text-slate-400 mt-1 pt-1.5 border-t border-slate-100 dark:border-slate-700">
            Total: PKR {card.parts.reduce((s, p) => s + p.line_total, 0).toLocaleString()}
          </div>
        </div>
      )}
